...
%% Клиентский доступ
%% =========================
U["Пользователь / браузер / мобильный клиент"]
DNS["DNS resale.ru"]

    CDN["CDN<br/>www.resale.ru / static.resale.ru / img.resale.ru<br/>SPA + статический контент + изображения"]

    U --> DNS
    DNS -->|"www / static / img"| CDN

    %% =========================
    %% Внешний API-контур
    %% =========================
    subgraph EDGE["Внешний контур"]
        L4A["L4 Cloud.ru LB<br/>api.resale.ru"]
        L4S["L4 Cloud.ru LB<br/>seller.resale.ru"]

        L7A["L7 NGINX group api<br/>5 нод, N+1<br/>SSL termination + routing"]
        L7S["L7 NGINX group seller<br/>4 ноды, N+1<br/>SSL termination + routing"]

        L4A --> L7A
        L4S --> L7S
    end

    DNS -->|"api.resale.ru"| L4A
    DNS -->|"seller.resale.ru"| L4S

    %% =========================
    %% Kubernetes
    %% =========================
    subgraph K8S["Kubernetes cluster (1 ДЦ, 3 группы узлов)"]
        APIGW["API Gateway"]
        AUTH["Auth / OAuth2<br/>JWT / access control"]

        APPS["Сервисы приложения<br/>Buyer API · Seller API · Search · Recommendation · Moderation · Workers"]

        subgraph NG1["Группа узлов 1"]
            G1["Backend-сервисы группы 1"]
        end

        subgraph NG2["Группа узлов 2"]
            G2["Backend-сервисы группы 2"]
        end

        subgraph NG3["Группа узлов 3"]
            G3["Backend-сервисы группы 3"]
        end
    ...
    %% =========================
    subgraph DATA["Контуры данных и инфраструктуры"]
        subgraph PG["PostgreSQL-контур<br/>coredb / cardsdb / mediadb / favoritesdb"]
            PGRW["PgBouncer RW<br/>запись"]
            PGRO["PgBouncer RO<br/>чтение"]

            PGP["Primary / shard primaries"]
            PGR["Replicas / shard replicas"]
        ...
        OS["OpenSearch<br/>card_search"]
        REDIS["Redis + Sentinel<br/>cache + user embeddings"]
        KAFKA["Kafka<br/>cards-upsert / cards-delete / user-actions<br/>moderation-events / media-events"]
        CH["ClickHouse<br/>event_log"]
        CEPH["Ceph RGW<br/>media + static origin"]
    end

    %% =========================
    %% Потоки данных
    %% =========================
    AUTH -->|данные пользователей / авторизация| PGRW
    AUTH -->|чтение| PGRO

    APPS -->|OLTP write| PGRW
    APPS -->|OLTP read| PGRO
    APPS -->|поиск / retrieval| OS
    APPS -->|cache| REDIS
    APPS -->|события| KAFKA
    APPS -->|загрузка фото| CEPH

    KAFKA -->|search-index-updater| OS
    KAFKA -->|feature-updater| REDIS
    KAFKA -->|ingest| CH
    KAFKA -->|media worker| CEPH

    CDN -->|cache miss / origin pull| CEPH
```
## Источники
