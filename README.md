...
Глобальной балансировки между датацентрами нет, поэтому DNS-схема сводится к маршрутизации на MSK и выделению CDN-доменов для статики и медиа. Подходы Geo-based / Latency-based DNS и ограничения DNS (например, кэширование и TTL) рассматриваются в лекции. [[15]](./20260220_highload_l3_v20_hse.pdf)
...
Anycast между датацентрами не используется, так как датацентр один. Концепция Anycast и типичный сценарий применения в CDN описаны в справке Cloudflare. [[14]](https://www.cloudflare.com/learning/cdn/glossary/anycast-network/)

Anycast может применяться на стороне CDN-провайдера для доменов:
- `img.resale.ru`
- `static.resale.ru`

### 3.6 Регулировка трафика между ДЦ

Регулировка трафика между датацентрами отсутствует, так как датацентр один.
