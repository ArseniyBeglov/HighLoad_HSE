...
|---|---|---|---|
| Eligibility | Географический и категорийный отбор + фильтр по статусу | Первичный отсев объявлений | До ranking в систему должны попадать только релевантные и доступные объявления |
| Базовый non-personalized layer | Геопопулярность + свежесть | Холодный старт, неавторизованная лента, fallback | Простая реализация, низкая стоимость online-path, естественный учёт географии и обновляемости каталога |
| Основной personalized candidate generation | Content-based recommendation | Генерация персонализированных кандидатов | Работает без длинной пользовательской истории и хорошо подходит для новых объявлений [[33]](https://developers.google.com/machine-learning/recommendation/content-based/basics) |
...
| Финальный ranking | `CatBoostRanker` | Ранжирование уже отобранных кандидатов | Подходит для табличных и категориальных признаков, поддерживает ranking objectives и удобен в эксплуатации [[34]](https://catboost.ai/docs/en/concepts/loss-functions-ranking) |
| Архитектурный паттерн | Двухэтапная схема: candidate generation + ranking | Общая схема рексистемы | Соответствует highload и позволяет улучшать качество без смены архитектуры [[30]](https://research.google/pubs/deep-neural-networks-for-youtube-recommendations/) [[31]](https://developers.google.com/machine-learning/recommendation/overview/candidate-generation) |

### 7.2 Итоговая сводная таблица по поиску

| Подсистема | Итоговый выбор | Роль в системе | Почему выбран |
|---|---|---|---|
| Retrieval | Inverted index + BM25 в OpenSearch | Базовый полнотекстовый поиск | Хорошо работает на коротких товарных запросах и естественно масштабируется [[35]](https://docs.opensearch.org/latest/search-plugins/keyword-search/) |
| Query normalization | `synonym` / `synonym_graph` + fuzzy fallback | Нормализация запроса и повышение recall | Улучшает поиск по сокращениям, разговорным названиям и опечаткам [[37]](https://docs.opensearch.org/latest/query-dsl/term/fuzzy/) [[39]](https://docs.opensearch.org/latest/analyzers/token-filters/synonym/) [[40]](https://docs.opensearch.org/latest/analyzers/token-filters/synonym-graph/) |
...
| Геозависимость | `geo_distance` filter + distance boost | Учёт близости продавца к покупателю | Соответствует ключевому продуктовому требованию [[38]](https://docs.opensearch.org/latest/query-dsl/geo-and-xy/geodistance/) [[36]](https://docs.opensearch.org/latest/query-dsl/compound/function-score/) |
| Финальный ranking | BM25 + `function_score` | Учёт свежести, расстояния, доверия к продавцу, popularity | Управляемый и дешёвый online-ranking для MVP [[36]](https://docs.opensearch.org/latest/query-dsl/compound/function-score/) |
| Этап 2 | Hybrid BM25 + vector ANN | Улучшение recall на длинном хвосте и семантических запросах | Имеет смысл только после стабилизации базового поиска [[41]](https://docs.opensearch.org/latest/tutorials/vector-search/neural-search-tutorial/) [[42]](https://docs.opensearch.org/latest/query-dsl/compound/hybrid/) |
...
| Content-based recommendation | Похожие товары, fallback при нехватке истории, cold start для новых объявлений | Кандидаты подбираются по признакам объявления: категория, бренд, ценовой диапазон, состояние, текст заголовка, атрибуты и география. | Работает без длинной истории, хорошо подходит для classifieds | Есть риск однообразной выдачи | Основной personalized generator в MVP [[33]](https://developers.google.com/machine-learning/recommendation/content-based/basics) |
...
| Two-tower retrieval | Большой каталог и зрелая ML-инфраструктура | Пользователь и объявление кодируются в embedding space, retrieval выполняется по nearest neighbors. | Сильная персонализация на больших данных | Сложная инфраструктура и дорогая эксплуатация | Для MVP избыточен; разумен как следующий этап [[30]](https://research.google/pubs/deep-neural-networks-for-youtube-recommendations/) |
| `CatBoostRanker` | Финальное ранжирование уже отобранных кандидатов | На вход подаются кандидаты от нескольких генераторов, далее модель ранжирует их по расстоянию, свежести, цене, качеству продавца, CTR, favorite rate, contact rate и другим признакам. | Хорошее качество на tabular features, удобная эксплуатация | Нужны корректные лейблы и feature pipeline | Лучший финальный ranker для проекта [[34]](https://catboost.ai/docs/en/concepts/loss-functions-ranking) |

### 7.4 Итоговый алгоритм рекомендательной системы

Для проекта **ReSale** в MVP используется следующий алгоритм рекомендательной системы:

**`eligibility -> (геопопулярность + свежесть + content-based candidate generation) -> CatBoostRanker`**

#### 7.4.1 Порядок работы алгоритма

| Шаг | Операция | Описание |
|---|---|---|
| 1 | Eligibility | Из общего каталога отбираются только объявления, допустимые по статусу, категории и географии |
| 2 | Candidate generation 1 | Формируется базовый набор кандидатов по алгоритму **геопопулярность + свежесть** |
| 3 | Candidate generation 2 | Формируется персонализированный набор кандидатов по **content-based recommendation** |
| 4 | Merge candidates | Оба набора кандидатов объединяются в общий пул |
| 5 | Ranking | Общий пул кандидатов ранжируется моделью **`CatBoostRanker`** |
| 6 | Final output | Пользователю показываются объявления с максимальным итоговым score |

#### 7.4.2 Признаки финального ранжирования

| Группа признаков | Примеры |
|---|---|
| География | расстояние до продавца, совпадение города, совпадение региона |
| Объявление | категория, бренд, состояние, цена, отклонение цены от медианы категории |
| Продавец | рейтинг, верификация, число жалоб |
| Поведение | CTR, favorite rate, contact rate, просмотры |
| Свежесть | время с публикации, время с последнего обновления |

#### 7.4.3 Итоговый выбор

Итоговый алгоритм рекомендаций для MVP:

**`eligibility -> (геопопулярность + свежесть + content-based candidate generation) -> CatBoostRanker`**

Этот вариант выбран, потому что он:

- закрывает cold start;
- учитывает географию;
- не требует длинной пользовательской истории;
...
| Inverted index + BM25 | Основной полнотекстовый поиск | Объявления индексируются в `card_search`, поиск выполняется по inverted index, базовый score считает BM25. | Хорошо работает на коротких товарных запросах, быстрый response time | Не учитывает бизнес-сигналы сам по себе | Лучший базовый retrieval для MVP [[35]](https://docs.opensearch.org/latest/search-plugins/keyword-search/) |
| Fuzzy query | Опечатки и неточные запросы | К лексическому поиску добавляется fuzzy fallback по edit distance. | Повышает recall на опечатках | При агрессивной настройке растёт шум | Нужен как ограниченный fallback [[37]](https://docs.opensearch.org/latest/query-dsl/term/fuzzy/) |
| `synonym` / `synonym_graph` | Синонимы, сокращения, разговорные названия | Используются словари вида `айфон -> iphone`, `ноут -> ноутбук`, `вел -> велосипед`. Для многословных выражений применяется `synonym_graph`. | Улучшает recall без изменения данных | Требует ручного управления словарями | Обязательный слой нормализации запросов [[39]](https://docs.opensearch.org/latest/analyzers/token-filters/synonym/) [[40]](https://docs.opensearch.org/latest/analyzers/token-filters/synonym-graph/) |
| `geo_distance` filter | Геозависимая выдача | Ограничивает результаты по радиусу относительно пользователя. | Напрямую соответствует продуктовой логике | Требует корректного `geo_point` поля | Обязательный географический фильтр [[38]](https://docs.opensearch.org/latest/query-dsl/geo-and-xy/geodistance/) |
| BM25 + `function_score` | Финальный online-ranking в поиске | К базовому BM25 добавляются функции для distance, freshness, seller quality и popularity. | Управляемый и дешёвый ranking | Требует ручного подбора весов | Лучший search ranking для MVP [[36]](https://docs.opensearch.org/latest/query-dsl/compound/function-score/) |
| Hybrid BM25 + vector ANN | Следующий этап развития поиска | Объединяет lexical retrieval и vector ANN retrieval. | Улучшает recall на длинных и семантических запросах | Сложнее эксплуатация и отладка | Не является стартовым вариантом [[41]](https://docs.opensearch.org/latest/tutorials/vector-search/neural-search-tutorial/) [[42]](https://docs.opensearch.org/latest/query-dsl/compound/hybrid/) |

### 7.6 Итоговый алгоритм поиска

Для проекта **ReSale** в MVP используется следующий алгоритм поиска:

**`query normalization -> BM25 retrieval -> filters -> geo_distance -> function_score ranking`**

#### 7.6.1 Порядок работы алгоритма

| Шаг | Операция | Описание |
|---|---|---|
| 1 | Query normalization | К запросу применяются словари `synonym` / `synonym_graph`; при необходимости включается fuzzy fallback |
| 2 | Retrieval | OpenSearch выполняет поиск по inverted index в индексе `card_search`; базовая текстовая релевантность считается алгоритмом **BM25** |
| 3 | Filtering | К найденным документам применяются обязательные фильтры: категория, цена, состояние, статус объявления, атрибуты |
| 4 | Geo filter | Применяется `geo_distance` для ограничения радиуса относительно пользователя |
| 5 | Ranking | Итоговый score считается через **`function_score`**, куда добавляются distance, freshness, seller trust и popularity |
| 6 | Final output | Пользователю возвращаются объявления с максимальным итоговым score |

#### 7.6.2 Итоговый выбор

Итоговый алгоритм поиска для MVP:
...
30. https://research.google/pubs/deep-neural-networks-for-youtube-recommendations/
31. https://developers.google.com/machine-learning/recommendation/overview/candidate-generation
32. https://www.cs.umd.edu/~samir/498/Amazon-Recommendations.pdf
33. https://developers.google.com/machine-learning/recommendation/content-based/basics
34. https://catboost.ai/docs/en/concepts/loss-functions-ranking
35. https://docs.opensearch.org/latest/search-plugins/keyword-search/
36. https://docs.opensearch.org/latest/query-dsl/compound/function-score/
37. https://docs.opensearch.org/latest/query-dsl/term/fuzzy/
38. https://docs.opensearch.org/latest/query-dsl/geo-and-xy/geodistance/
39. https://docs.opensearch.org/latest/analyzers/token-filters/synonym/
40. https://docs.opensearch.org/latest/analyzers/token-filters/synonym-graph/
41. https://docs.opensearch.org/latest/tutorials/vector-search/neural-search-tutorial/
42. https://docs.opensearch.org/latest/query-dsl/compound/hybrid/
43. https://go.dev/doc/
